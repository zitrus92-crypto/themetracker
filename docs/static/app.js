...
// --- Finviz link ---
// Same filter + sort as the Themes links (f=theme_<slug>,ta_volatility_mo3&o=-perf13w),
...
function finvizUrl(ticker) {
  if (!ticker) return "";
  return `https://finviz.com/screener.ashx?v=211&f=ind_${ticker}%2Cta_volatility_mo3&o=-perf13w`;
}
...
function themeScreenerUrl(theme) {
  ...
  return `https://finviz.com/screener.ashx?v=211&f=theme_${slug}%2Cta_volatility_mo3&o=-perf13w`;
}
...
function renderEtfThemes(data) {
  ...
  const rows = entries.map(([theme, row], idx) => {
    ...
    const chips = (row.top3 || []).map(nodeKey => {
      ...
      const url = `https://finviz.com/screener.ashx?v=211&f=subtheme_${nodeKey}%2Cta_volatility_mo3&o=-perf13w`;
...
function renderEtfList(data) {
  ...
  const rows = entries.map(([key, row], idx) => {
    ...
    const subUrl = `https://finviz.com/screener.ashx?v=211&f=subtheme_${key}%2Cta_volatility_mo3&o=-perf13w`;
